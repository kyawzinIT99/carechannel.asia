...
export async function VisitAssistSection({
  ...
  const [pickupTitle, pickupBody, stayTitle, stayBody] = await Promise.all([
    ...
    <section className={compact ? "" : "py-4"}>
      <div className={compact ? "" : "mx-auto max-w-6xl"}>
        ...
        <h2 className="mt-1 text-2xl font-semibold text-[#1a2330] md:text-3xl">
          {my ? "လေဆိပ်ကားနှင့် နေထိုင်ရန် အကူအညီ" : "Airport pickup and stay help"}
        </h2>
        <p className="mt-2 max-w-2xl text-sm leading-7 text-slate-500">
          {my
            ? "ဤအကူအညီများသည် စစ်ဆေးပက်ကေ့ချ်တွင် မပါဝင်ပါ။ လိုမှသာ ညှိနှိုင်းရေးမှူးက ကူညီသည်။ နေထိုင်ရန်သည် ဟိုတယ်ပက်ကေ့ချ် မဟုတ်ပါ။"
            : "This is optional help only — not part of a checkup package. A stay is a simple rental apartment if you want one, not a hotel package."}
        </p>
        <div className="mt-6 grid gap-4 md:grid-cols-2">
          {cards.map((item) => (
            <article key={item.code} className="rounded-3xl bg-white p-6 shadow-sm ring-1 ring-slate-100">
              <p className="text-xs font-bold uppercase tracking-widest text-sky-700">{item.kind}</p>
        ...
        >
          {my ? "လိုပါက ကြိုဆိုကား / နေထိုင်ရန် ပြောမည်" : "Ask for pickup or a stay if you want"}
        </Link>
      </div>
    </section>
  );
